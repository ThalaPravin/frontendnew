...
import Main from "../otpcomponent/Main.tsx";
export default function Profilesettings() {
	...
	const handlemedicalHistory = () => {
		setFormData((prevState) => ({
			...prevState,
			medicalHistory: [...prevState.medicalHistory, { diseaseName: "", year: "" }], // Add a new empty allergy field
		}));
	};
	...
	const getPatientInfo = async () => {
		...
		if (patientInfo) {
			setFormData({
				...
				profilePicture:
					patientInfo?.profilePicture || "/assets/img/banners/defualtImgjpg.jpg",
				mobile: patientInfo?.userId?.mobileNumber || "",
				emergencyNumber: patientInfo?.emergencyNumber || "",
				dob: patientInfo?.dob || "",
	...
	const handleSubmit = async (e) => {
		...
		try {
			if (patientInfo) {
				...
				localStorage.setItem("patientInfo", JSON.stringify(updatedPatient.data.result));
				...
				const user = await axios.post(`http://localhost:5000/patient/create`, formData, {
					headers: {
						"Content-Type": "application/json",
						Authorization: isAuthenticated,
					},
				...
				<div className="modal-dialog modal-dialog-centered" role="document">
					<div className="modal-content">
						<div className="modal-header">
							<h3 className="modal-title">Verify Mobile No.</h3>
							<button
								type="button"
								className="btn-close"
