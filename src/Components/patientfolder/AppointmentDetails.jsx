...
const [currentReportType, setCurrentReportType] = useState("");
    ...
    if (currentReportType === "lab" && labFile) {
      try {
        ...
        const reportUploadResult = await axios.put(
          `https://healthcare-backend-o4vb.onrender.com/patient/${patientId}/report`,
          newReportFile,
          {
            headers: {
              isvalidrequest: "twinsistech",
  ...
  {currentReportType === "lab" && (
    <>
      {labFile && (
        <p>Selected file: {labFile.name}</p>
      )}
      <div className="mb-3">
        <label htmlFor="labDescriptionInput">Report Description:</label>
        <input
          type="text"
...
<div className="mb-3">
  ...
  <label htmlFor="radiologyFileInput">Upload Radiology Report:</label>
  <input
    type="file"
  ...
  {currentReportType === "radiology" && (
    <>
      ...
      <div className="mb-3">
        <label htmlFor="radiologyDescriptionInput">Report Description:</label>
        <input
          type="text"
          id="radiologyDescriptionInput"
          className="form-control"
          value={radiologyDescription}
          onChange={(e) => setRadiologyDescription(e.target.value)}
        />
      </div>
      <button type="button" className="btn btn-primary" onClick={handleUpload}>
...
{/* {reportFiles?.map((report) => (
									<iframe
										...
										src={`https://healthcare-backend-o4vb.onrender.com/files/${report.filename}`}
									></iframe>
								))} */}
  ...
  <div key={index}>
    <h5>{report.type === "lab" ? "Lab Report" : "Radiology Report"}</h5>
    <iframe
      style={{ width: "100%", height: "500px" }}
      src={`https://healthcare-backend-o4vb.onrender.com/files/${report.filename}`}
